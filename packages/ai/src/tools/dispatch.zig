...
pub fn execute(
    ...
    if (std.mem.eql(u8, call.name, "search")) {
        ...
        return .{ .text = allocator.dupe(u8, out.observation) catch return error.WorkspaceFailed };
    }
    if (std.mem.eql(u8, call.name, "codebase_search")) {
        ...
        if (out.formatted) |formatted| {
            return .{ .text = allocator.dupe(u8, formatted) catch return error.WorkspaceFailed };
        }
        return .{ .text = allocator.dupe(u8, out.summary) catch return error.WorkspaceFailed };
    }
    ...
    if (std.mem.eql(u8, call.name, "fetch_url")) {
        ...
        if (out.content) |content| {
            return .{ .text = allocator.dupe(u8, content) catch return error.WorkspaceFailed };
        }
        return .{ .text = allocator.dupe(u8, out.summary) catch return error.WorkspaceFailed };
    }
