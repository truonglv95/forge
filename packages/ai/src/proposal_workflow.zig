...
pub const default_agent_modify_sample_response =
    \\{"schema_version":1,"summary":"Add reliability marker to sample.txt","assumptions":["sample.txt matches eval seed"],"validation_tasks":["zig build test"],"workspace_edit":{"files":[{"path":"sample.txt","operation":"modify","edits":[{"start":0,"end":19,"replacement":"hello forge search\nreliability marker\n"}]}]}}
;
...
test "default modify sample response validates after live hash fill" {
    const allocator = std.testing.allocator;
    const io = std.testing.io;
    var tmp = std.testing.tmpDir(.{ .iterate = true, .access_sub_paths = true });
    defer tmp.cleanup();
    const root = workspace.WorkspaceRoot.init(tmp.dir, ".");
    try tmp.dir.writeFile(io, .{ .sub_path = "sample.txt", .data = "hello forge search\n" });

    const prepared = try proposal_precondition.fillMissingExpectedHashes(allocator, io, root, default_agent_modify_sample_response);
    defer allocator.free(prepared);
    try validateProposalBody(allocator, prepared);
}
