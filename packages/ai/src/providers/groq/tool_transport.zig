...
const groq_provider = @import("provider.zig");
...
/// Tool-loop transport for Groq. Groq is fully OpenAI-compatible at the
/// /chat/completions endpoint with `tools`, `tool_choice`, and SSE streaming.
/// Notable free-tier models: llama-3.3-70b-versatile, llama-3.1-8b-instant,
/// deepseek-r1-distill-llama-70b, qwen-2.5-coder-32b, moonshotai/kimi-k2-instruct.
pub const GroqTransport = struct {
    groq: *groq_provider.GroqProvider,
    io: std.Io,
    mcp: ?*mcp_registry.Registry,

    pub fn transport(self: *GroqTransport) turn.Transport {
        return .{
            .ptr = self,
    ...
    pub fn declarationsJson(self: *const GroqTransport, allocator: std.mem.Allocator) ![]const u8 {
    ...
    fn completeTurn(
        ...
        const self: *GroqTransport = @ptrCast(@alignCast(ptr));
        if (cancel_token) |token| if (token.isCancelled()) return error.Cancelled;

        var bridge = StreamBridge{ .groq = self.groq };
        ...
        fetchStreamChatInto(self.groq, allocator, self.io, conversation_json, tool_declarations_json, cancel_token, &parser) catch |err| return switch (err) {
        ...
        self.groq.latest_usage = parser.latest_usage;
...
const StreamBridge = struct {
    ...
    fn onChunk(context: ?*anyopaque, chunk: []const u8) void {
        ...
        if (bridge.groq.stream_callback) |callback| callback(bridge.groq.stream_context, chunk);
    }
};
...
fn fetchStreamChatInto(
    ...
    const endpoint = try groq_provider.buildChatEndpoint(allocator, groq.base_url);
    ...
    const model_escaped = try jsonString(allocator, groq.model_name);
    ...
    const auth = try std.fmt.allocPrint(allocator, "Bearer {s}", .{groq.creds.api_key});
...
test "jsonString escapes model ids" {
    const value = try jsonString(std.testing.allocator, "llama-3.3-70b-versatile");
    defer std.testing.allocator.free(value);
    try std.testing.expectEqualStrings("\"llama-3.3-70b-versatile\"", value);
}
