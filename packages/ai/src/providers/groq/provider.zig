...
const groq_transport = @import("tool_transport.zig");

/// Groq is a free-tier OpenAI-compatible inference provider running on LPU
/// hardware. Sign up at https://console.groq.com to get a free API key
/// (env var: GROQ_API_KEY). Default free tier: ~30 req/min, 14k req/day.
pub const default_base_url = "https://api.groq.com/openai/v1";
pub const base_url_env_var = "GROQ_BASE_URL";
...
pub const GroqProvider = struct {
    ...
    pub fn create(
        ...
        const ptr = try allocator.create(GroqProvider);
        ptr.* = .{
            .allocator = allocator,
            .io = io,
            .creds = creds,
    ...
    pub fn deinit(ptr: *anyopaque, allocator: std.mem.Allocator) void {
        const self: *GroqProvider = @ptrCast(@alignCast(ptr));
        self.creds.deinit();
    ...
    pub fn providerInterface(self: *GroqProvider) provider.Provider {
        return .{
            .ptr = self,
            .vtable = &.{
                .ask = askImpl,
    ...
    fn metadataImpl(ptr: *const anyopaque) provider.ModelMetadata {
        const self: *const GroqProvider = @ptrCast(@alignCast(ptr));
        return self.meta;
    }
    ...
    fn toolTransportState(self: *GroqProvider, io: std.Io, mcp: ?*mcp_registry.Registry) groq_transport.GroqTransport {
        return .{ .groq = self, .io = io, .mcp = mcp };
    }
...
test "buildChatEndpoint appends OpenAI-compatible route" {
    const endpoint = try buildChatEndpoint(std.testing.allocator, "https://api.groq.com/openai/v1/");
    defer std.testing.allocator.free(endpoint);
    try std.testing.expectEqualStrings("https://api.groq.com/openai/v1/chat/completions", endpoint);
}

test "buildRequestPayload requests json_schema for proposals" {
    const payload = try buildRequestPayload(std.testing.allocator, "llama-3.3-70b-versatile", "Output ONLY a raw JSON object WorkspaceEdit");
