...
const cerebras_provider = @import("provider.zig");
...
/// Tool-loop transport for Cerebras. Cerebras is fully OpenAI-compatible at
/// the /chat/completions endpoint with `tools`, `tool_choice`, and SSE streaming.
/// Notable free-tier models: llama-3.3-70b, llama3.1-8b, qwen-3-coder-30b.
pub const CerebrasTransport = struct {
    cerebras: *cerebras_provider.CerebrasProvider,
    io: std.Io,
    mcp: ?*mcp_registry.Registry,

    pub fn transport(self: *CerebrasTransport) turn.Transport {
        return .{
            .ptr = self,
    ...
    pub fn declarationsJson(self: *const CerebrasTransport, allocator: std.mem.Allocator) ![]const u8 {
    ...
    fn completeTurn(
        ...
        const self: *CerebrasTransport = @ptrCast(@alignCast(ptr));
        if (cancel_token) |token| if (token.isCancelled()) return error.Cancelled;

        var bridge = StreamBridge{ .cerebras = self.cerebras };
        ...
        fetchStreamChatInto(self.cerebras, allocator, self.io, conversation_json, tool_declarations_json, cancel_token, &parser) catch |err| return switch (err) {
        ...
        self.cerebras.latest_usage = parser.latest_usage;
...
const StreamBridge = struct {
    ...
    fn onChunk(context: ?*anyopaque, chunk: []const u8) void {
        ...
        if (bridge.cerebras.stream_callback) |callback| callback(bridge.cerebras.stream_context, chunk);
    }
};
...
fn fetchStreamChatInto(
    ...
    const endpoint = try cerebras_provider.buildChatEndpoint(allocator, cerebras.base_url);
    ...
    const model_escaped = try jsonString(allocator, cerebras.model_name);
    ...
    const auth = try std.fmt.allocPrint(allocator, "Bearer {s}", .{cerebras.creds.api_key});
...
test "jsonString escapes model ids" {
    const value = try jsonString(std.testing.allocator, "llama-3.3-70b");
    defer std.testing.allocator.free(value);
    try std.testing.expectEqualStrings("\"llama-3.3-70b\"", value);
}
