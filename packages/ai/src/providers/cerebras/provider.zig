...
const cerebras_transport = @import("tool_transport.zig");

/// Cerebras is a free-tier OpenAI-compatible inference provider running on
/// wafer-scale CS-3 chips ( fastest inference for open models — up to ~2k
/// tokens/sec on Llama 3.3 70B ). Sign up at https://cloud.cerebras.ai to
/// get a free API key (env var: CEREBRAS_API_KEY).
/// Free tier: ~1M tokens/day, 30 RPM, no credit card required.
pub const default_base_url = "https://api.cerebras.ai/v1";
pub const base_url_env_var = "CEREBRAS_BASE_URL";
...
pub const CerebrasProvider = struct {
    ...
    pub fn create(
        ...
        const ptr = try allocator.create(CerebrasProvider);
        ptr.* = .{
            .allocator = allocator,
            .io = io,
            .creds = creds,
    ...
    pub fn deinit(ptr: *anyopaque, allocator: std.mem.Allocator) void {
        const self: *CerebrasProvider = @ptrCast(@alignCast(ptr));
        self.creds.deinit();
    ...
    pub fn providerInterface(self: *CerebrasProvider) provider.Provider {
        return .{
            .ptr = self,
            .vtable = &.{
                .ask = askImpl,
    ...
    fn metadataImpl(ptr: *const anyopaque) provider.ModelMetadata {
        const self: *const CerebrasProvider = @ptrCast(@alignCast(ptr));
        return self.meta;
    }
    ...
    fn toolTransportState(self: *CerebrasProvider, io: std.Io, mcp: ?*mcp_registry.Registry) cerebras_transport.CerebrasTransport {
        return .{ .cerebras = self, .io = io, .mcp = mcp };
    }
...
test "buildChatEndpoint appends OpenAI-compatible route" {
    const endpoint = try buildChatEndpoint(std.testing.allocator, "https://api.cerebras.ai/v1/");
    defer std.testing.allocator.free(endpoint);
    try std.testing.expectEqualStrings("https://api.cerebras.ai/v1/chat/completions", endpoint);
}

test "buildRequestPayload requests json_schema for proposals" {
    const payload = try buildRequestPayload(std.testing.allocator, "llama-3.3-70b", "Output ONLY a raw JSON object WorkspaceEdit");
