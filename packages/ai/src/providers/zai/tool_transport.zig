...
const zai_provider = @import("provider.zig");
...
/// internal-api.z.ai/v1/chat/completions is fully OpenAI-compatible:
///   - Accepts `tools` (array of {type:"function",function:{name,description,parameters}})
///   - Accepts `tool_choice: "auto"`
///   - Returns `tool_calls` in the assistant message
///   - Accepts `role:"tool"` messages with `tool_call_id` for results
///   - Supports SSE streaming with `delta.content` chunks
pub const ZaiTransport = struct {
    zai: *zai_provider.ZaiProvider,
    io: std.Io,
    mcp: ?*mcp_registry.Registry,

    pub fn transport(self: *ZaiTransport) turn.Transport {
        return .{
            .ptr = self,
    ...
    pub fn declarationsJson(self: *const ZaiTransport, allocator: std.mem.Allocator) ![]const u8 {
    ...
    fn completeTurn(
        ...
        const self: *ZaiTransport = @ptrCast(@alignCast(ptr));
        if (cancel_token) |token| if (token.isCancelled()) return error.Cancelled;

        var bridge = StreamBridge{ .zai = self.zai };
        ...
        fetchStreamChatInto(self.zai, allocator, self.io, conversation_json, tool_declarations_json, cancel_token, &parser) catch |err| return switch (err) {
        ...
        self.zai.latest_usage = parser.latest_usage;
...
const StreamBridge = struct {
    ...
    fn onChunk(context: ?*anyopaque, chunk: []const u8) void {
        ...
        if (bridge.zai.stream_callback) |callback| callback(bridge.zai.stream_context, chunk);
    }
};
...
fn fetchStreamChatInto(
    ...
    const endpoint = try zai_provider.buildChatEndpoint(allocator, zai.base_url);
    ...
    const headers = [_]std.http.Header{
        ...
        .{ .name = "X-Token", .value = zai.creds.api_key },
...
test "jsonString escapes model ids" {
    const value = try jsonString(std.testing.allocator, "glm-4-plus");
    defer std.testing.allocator.free(value);
    try std.testing.expectEqualStrings("\"glm-4-plus\"", value);
}
