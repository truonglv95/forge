...
pub const OllamaTransport = struct {
    ...
    fn appendAssistantToolCall(
        ...
        try piece.appendSlice(allocator, if (validJson(allocator, call.args_json)) call.args_json else "{}");
    ...
    fn appendToolResult(
        ...
        defer allocator.free(escaped);
        const piece = try std.fmt.allocPrint(allocator, "{{\"role\":\"tool\",\"content\":{s},\"tool_name\":\"{s}\"}}", .{ escaped, tool_name });
        defer allocator.free(piece);
        try conversation.appendSlice(allocator, piece);
    }
};
