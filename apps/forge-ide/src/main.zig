...
pub fn main(init: std.process.Init) !void {
    ...
    panel_view.?.addChild(gpa, border_view.?) catch |e| { std.debug.print("OOM: {}\n", .{e}); };
...
fn isKeyword(word: []const u8) bool {
    const keywords = [_][]const u8{
        "pub", "fn", "const", "var", "struct", "enum", "union",
        "return", "try", "catch", "if", "else", "switch", "while", "for",
        "break", "continue", "defer", "errdefer", "and", "or", "true", "false",
        "void", "bool", "f32", "f64", "i8", "u8", "i32", "u32", "i64", "u64", "usize", "isize"
    };
    for (keywords) |kw| {
