...
pub const markdown = struct {
    pub const default_body_font_size: f32 = 14.5;
    ...
    pub var paragraph_gap: f32 = 14.0;
    pub var list_item_gap: f32 = 6.0;
...
pub fn configureAiPanelFontSize(raw_size: f32) void {
    ...
    markdown.body_line_h = @round(size * 1.62);
    markdown.code_font_size = @max(11.0, size - 1.5);
    markdown.code_line_h = @round(markdown.code_font_size * 1.65);
    markdown.heading_font_size = size + 1.0;
    markdown.heading_line_h = @round(markdown.heading_font_size * 1.62);
    markdown.paragraph_gap = @round(size * 0.95);
    markdown.list_item_gap = @round(size * 0.42);
    markdown.heading_gap_top = @round(size * 1.05);
    markdown.heading_gap_bottom = @round(size * 0.68);
