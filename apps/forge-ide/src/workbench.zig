...
pub const Workbench = struct {
    ...
    pub fn initWithOptions(self: *Workbench, allocator: std.mem.Allocator, io: std.Io, workspace_path: []const u8, ide_launcher: []const u8, environ_map: ?*const std.process.Environ.Map, options: InitOptions) !void {
        ...
        // Sample agent message injection is disabled — it was causing
        // heap corruption when the markdown renderer tried to parse the
        // multi-line string literal. The sample can be re-enabled once
        // the root cause of the corruption is identified.
