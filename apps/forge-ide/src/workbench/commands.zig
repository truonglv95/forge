const std = @import("std");

pub const BottomPanelMode = enum { output, problems, terminal, debug_console, debug_variables, debug_callstack };

pub const Command = union(enum) {
    open_file: []const u8,
    open_settings: void,
    activate_tab: usize,
    close_tab: usize,
    close_active_tab: void,
    close_all_tabs: void,
    save_active: void,
    explorer_toggle: []const u8,
    explorer_select: []const u8,
    explorer_create_file: []const u8,
    explorer_create_folder: []const u8,
    explorer_rename: struct {
        path: []const u8,
        new_name: []const u8,
    },
    explorer_delete_selected: void,
    explorer_begin_rename: void,
    run_extension_command: []const u8,
    reload_extensions: void,
    refresh_explorer: void,
    select_output_channel: void,
    run_task: []const u8,
    check_external_conflicts: void,
    reload_active_from_disk: void,
    dismiss_external_conflict: void,
    restore_recovery_snapshots: void,
    discard_recovery_snapshots: void,
    agent_set_mode: @import("../agent/session.zig").Mode,
    agent_submit: void,
    agent_edit_selection: void,
    agent_cancel: void,
    agent_apply: void,
    agent_rollback: void,
    agent_dismiss_apply: void,
    agent_approve_spec: void,
    agent_approve_tool: void,
    agent_approve_always_tool: void,
    agent_reject_tool: void,
    agent_continue_session: void,
    agent_dismiss_resume: void,
    agent_reject: void,
    agent_show_review: void,
    agent_toggle_step: usize,
    agent_select_run: usize,
    agent_refresh_runs: void,
    agent_add_scope: []const u8,
    agent_remove_scope: []const u8,
    agent_clear_scope: void,
    agent_scope_picker_open: void,
    agent_scope_picker_close: void,
    agent_toggle_mode_menu: void,
    agent_toggle_model_menu: void,
    agent_close_menus: void,
    agent_set_model: usize,
    agent_remove_attachment: usize,
    agent_copy_message: usize,
    agent_open_message: usize,
    agent_toggle_context_inspector: void,
    agent_scope_picker_select: void,
    set_shell_mode: @import("../ui/core/layout.zig").ShellMode,
    toggle_shell_mode: void,
    reload_theme: void,
    set_sidebar_view: @import("../ui/sidebar/sidebar_view.zig").SidebarView,
    extension_toggle: usize,
    open_extensions_dir: []const u8,
    install_marketplace_extension: []const u8,
    apply_extension_theme: []const u8,
    set_extensions_panel_mode: @import("../ui/sidebar/extensions_panel.zig").PanelMode,
    search_run: void,
    git_refresh: void,
    git_pull: void,
    git_push: void,
    git_stage_all: void,
    git_unstage_all: void,
    git_stash: void,
    git_stash_pop: void,
    git_create_branch: void,
    git_log: void,
    git_blame: void,
    file_quick_open: void,
    workspace_replace_all: void,
    diff_open: []const u8,
    snippet_insert: []const u8,
    profile_save: void,
    profile_load: void,
    terminal_split: void,
    terminal_focus_switch: void,
    keybinding_open_editor: void,
    extension_open_settings: void,
    ai_fix_diagnostic: void,
    ai_implement_todo: void,
    ai_review_selection: void,
    ai_toggle_proactive_completion: void,
    ai_auto_context: void,
    undo_history_show: void,
    semantic_search_panel: void,
    uninstall_extension: []const u8,
    extensions_show_detail: usize,
    extensions_back_from_detail: void,
    set_bottom_panel_mode: BottomPanelMode,
    terminal_submit: void,
    terminal_new: void,
    terminal_close: void,
    terminal_next: void,
    terminal_prev: void,
    terminal_activate: usize,
    debug_toggle_breakpoint: void,
    debug_clear_breakpoints: void,
    debug_run_launch: usize,
    debug_clear_console: void,
    debug_continue: void,
    debug_step_over: void,
    debug_step_into: void,
    debug_step_out: void,
    debug_stop: void,
    editor_completion: void,
    editor_find: void,
    editor_replace: void,
    editor_goto_line: void,
    editor_find_next: void,
    editor_find_prev: void,
    editor_find_close: void,
    editor_redo: void,
    editor_undo: void,
    editor_scroll_to_cursor: void,
    editor_go_to_definition: void,
    editor_find_references: void,
    editor_rename_symbol: void,
    editor_format_document: void,
    editor_split_right: void,
    editor_close_split: void,
    editor_accept_inline_edit: void,
    editor_reject_inline_edit: void,
    rename_accept: void,
    rename_reject: void,
    references_goto: usize,
    problems_goto: usize,
    completion_accept: void,
    completion_dismiss: void,
    save_session_state: void,
    restore_session_state: void,
    settings_reload: void,
    settings_toggle_word_wrap: void,
    settings_toggle_minimap: void,
    settings_toggle_theme: void,
    open_recent_workspace: usize,
    problem_quick_fix: void,
    debug_stack_goto: usize,
    debug_copy_variable: usize,
    toggle_sidebar: void,
    toggle_bottom_panel: void,
    toggle_agent_panel: void,
    focus_agent: void,
    chat_clear_history: void,
    close_proposal_review: void,
    close_settings_modal: void,
    nav_back: void,
    nav_forward: void,
    open_settings_modal: void,
    ai_open_settings_toml: void,
    ai_open_mcp_config: void,
    ai_toggle_mcp: void,
    ai_refresh_mcp: void,
    ai_edit_provider: void,
    ai_edit_model: void,
    ai_edit_embedding_provider: void,
    ai_edit_embedding_model: void,
    ai_set_embedding_model: usize,
    ai_model_select: struct { kind: @import("ai_model_config.zig").ModelKind, index: usize },
    ai_model_add: @import("ai_model_config.zig").ModelKind,
    ai_model_edit: struct { kind: @import("ai_model_config.zig").ModelKind, index: usize },
    ai_model_delete: struct { kind: @import("ai_model_config.zig").ModelKind, index: usize },
    ai_toggle_hyde: void,
    palette_open: void,
    palette_close: void,
    palette_git_switch_branch: void,
    workspace_symbol_picker_open: void,
    workspace_symbol_picker_close: void,
    workspace_symbol_picker_select: void,
    // Ghost text (inline AI completion)
    ghost_completion_accept: void,
    ghost_completion_dismiss: void,
    ghost_completion_toggle: void,
    // Multi-cursor (P0-4)
    editor_add_cursor_next: void,
    editor_add_cursor_all: void,
    editor_clear_cursors: void,
    // Code folding (P0-4)
    editor_fold_toggle: void,
    editor_fold_all: void,
    editor_unfold_all: void,
    // Context menu + quick fix lightbulb (P0-5)
    editor_open_context_menu: void,
    editor_apply_quick_fix: usize,
    editor_show_quick_fixes: void,
    // Cmd+K inline edit (P0-2)
    inline_edit_open: void,
    inline_edit_submit: void,
    inline_edit_accept: void,
    inline_edit_reject: void,
    inline_edit_cancel: void,
    // Multi-file Composer (Cursor parity) — add/remove files from the
    // current inline edit batch so the agent edits across multiple files.
    composer_add_active_file: void,
    composer_remove_file: usize,
    composer_clear_files: void,
    // @mentions (P0-3)
    chat_mention_file: void,
    chat_mention_symbol: void,
    chat_mention_folder: void,
    chat_mention_web: void,
    chat_mention_select: usize,
    chat_mention_dismiss: void,
    // P1.5-3: Watch expressions
    debug_watch_add: []const u8,
    debug_watch_remove: usize,
    debug_watch_clear: void,
    debug_watch_refresh: void,
    // Conflict resolution
    conflict_accept_current: usize,
    conflict_accept_incoming: usize,
    conflict_accept_both: usize,
};

pub const Event = union(enum) {
    file_opened: []const u8,
    file_saved: []const u8,
    explorer_refreshed: void,
    extension_loaded: []const u8,
    status_message: []const u8,
    task_output: []const u8,
    task_finished: i32,
};

pub fn freeCommand(allocator: std.mem.Allocator, command: Command) void {
    switch (command) {
        .open_file, .explorer_toggle, .explorer_select => |path| allocator.free(path),
        .explorer_create_file, .explorer_create_folder, .run_task, .run_extension_command, .open_extensions_dir, .install_marketplace_extension, .apply_extension_theme, .uninstall_extension, .debug_watch_add => |owned| allocator.free(owned),
        .explorer_rename => |payload| allocator.free(payload.new_name),
        else => {},
    }
}
