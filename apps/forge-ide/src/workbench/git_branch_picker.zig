...
pub const Picker = struct {
    ...
    pub fn openPicker(self: *Picker, workspace_path: []const u8) !void {
        self.open = true;
        self.query_len = 0;
        self.selected = 0;
        try self.fetchBranches(workspace_path);
    ...
    pub fn fetchBranches(self: *Picker, workspace_path: []const u8) !void {
        self.clearEntries();
        const result = process_spawn.runCapture(self.allocator, &.{ "git", "branch", "--format=%(refname:short)" }, .{ .cwd = workspace_path }) catch return;
