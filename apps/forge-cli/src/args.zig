...
pub const GlobalFlags = struct {
    ...
    // Code review flags (forge review)
    staged: bool = false,
    base: ?[]const u8 = null,
    head: ?[]const u8 = null,
    heuristic_only: bool = false,
    // Multi-agent orchestration flags (forge agent run --coordinated)
    coordinated: bool = false,
    // Superpower Harness: enforce 7-phase workflow (Brainstorm→Spec→Plan→TDD→Subagent→Review→Finalize)
    harness: bool = false,
    // Parallel multi-agent: spawn N agents in parallel (Antigravity parity)
    parallel: bool = false,
};
        ...
        while (i < args.len) : (i += 1) {
            ...
            if (std.mem.startsWith(u8, arg, "-")) {
                if (std.mem.eql(u8, arg, "--json")) {
                    ...
                    flags.staged = true;
                } else if (std.mem.eql(u8, arg, "--heuristic-only")) {
                    flags.heuristic_only = true;
