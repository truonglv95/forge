...
pub const GlobalFlags = struct {
    ...
    // Cloud flags
    cloud: bool = false, // fetch models from cloud backend instead of builtin table
    // Rendering mode: --tui forces full-screen TUI (cursor positioning).
    // Default is inline mode (sequential output, native terminal scrollback).
    tui: bool = false,
};
